"""
The file to hold the Set PH Target class
"""

from src.ui_state.user_value import UserValue


class SetPHTarget(UserValue):
    """
    This is a class for the SetPHTarget state of the Tank Controller
    """

    def __init__(self, titrator, previous_state=None):
        """
        Constructor for the SetPHTarget class
        """
    ...
    def save_value(self):
        ...
        if self.sub_state < len(self.values):
            ...
            self.titrator.ph_control.set_ramp_duration_hours(self.values[1])

            ph_set_point = f"New pH={self.values[0]:.3f}"
            ramp_hours = f"New ramp={self.values[1]:.3f}"
            self.titrator.lcd.print(ph_set_point, line=1)
            self.titrator.lcd.print(ramp_hours, line=2)
