"""
Test suite for the SetPHTarget class
"""
...
def test_set_ph_target_handle_key(print_mock):
    """
    Test that handle_key processes key presses correctly.
    """
    titrator = Titrator()
    state = SetPHTarget(titrator, MainMenu(titrator))
...
def test_set_ph_target_advances_substates():
    """
...
def test_set_ph_target_valid_input(print_mock):
    ...
    titrator.ph_control.set_ramp_duration_hours.assert_called_once_with(2.0)

    print_mock.assert_any_call("New pH=7.500", line=1)
    print_mock.assert_any_call("New ramp=2.000", line=2)
    assert isinstance(titrator.state.next_state, MainMenu)


def test_set_ph_target_get_label():
    """
    ...
    assert state.get_label() == "Set pH Set Point:"
    state.sub_state = 1
