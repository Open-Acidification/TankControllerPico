...
from tank_controller.ui_state.demo_mode.read_values import ReadValues
...
class DemoModeMenu(UIState):
    ...
    def handle_key(self, key):
        ...
        if self.substate == 1:
            if key == Keypad.KEY_1:
                self._set_next_state(ReadValues(self.tank_controller, self), True)

            elif key == Keypad.KEY_2:
                self.substate = 3

            elif key == Keypad.KEY_3:
                self._set_next_state(DemoPump(self.tank_controller, self), True)
    ...
    def loop(self):
        ...
        """
        if self.substate == 1:
            self.tank_controller.lcd.print("1: Read values", line=1)
            self.tank_controller.lcd.print("2: Demo pH probe", line=2)
            self.tank_controller.lcd.print("3: Demo pump", line=3)
            self.tank_controller.lcd.print("4: Page 2", line=4)

        elif self.substate == 2:
            self.tank_controller.lcd.print("1: Demo stir control", line=1)
            self.tank_controller.lcd.print("2: Demo temp probe", line=2)
            self.tank_controller.lcd.print("3: Demo temp control", line=3)
