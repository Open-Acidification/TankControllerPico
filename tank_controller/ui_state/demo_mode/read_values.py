...
class ReadValues(UIState):
    ...
    def loop(self):
        ...
        if self.substate == 1:
            ...
            self.tank_controller.lcd.print(
                f"Gain:   {self.tank_controller.ph_probe.get_gain()}", line=3
            )
            self.tank_controller.lcd.print(
                f"Volume: {self.tank_controller.pump.get_volume_in_pump()} ml", line=4
            )

        elif self.substate == 2:
            self.tank_controller.lcd.print(
                f"Temp:   {self.tank_controller.temperature_probe_control.get_temperature():>4.3f} C",
                line=1,
            )
            self.tank_controller.lcd.print(
                f"Res:    {self.tank_controller.temperature_probe_control.get_resistance():>4.3f} Ohms",
                line=2,
            )
