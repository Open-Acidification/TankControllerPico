...
class PrimePump(UIState):
    ...
    def loop(self):
        ...
        """
        if self.substate == 1:
            self.tank_controller.lcd.print("How many pumps?", line=1)
            self.tank_controller.lcd.print("Choose a number", line=2)
            self.tank_controller.lcd.print("Choose 0 to return", line=3)
            self.tank_controller.lcd.print("", line=4)

        elif self.substate == 2:
            self.tank_controller.lcd.print("How many more?", line=1)
            self.tank_controller.lcd.print("Choose a number", line=2)
            self.tank_controller.lcd.print("Choose 0 to return", line=3)
            self.tank_controller.lcd.print("", line=4)
