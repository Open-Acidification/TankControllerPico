...
class MainMenu(UIState):
    ...
    def handle_key(self, key):
        ...
        if key == Keypad.KEY_A:  # Set pH set_point
            self._set_next_state(SetPHTarget(self.tank_controller, self), True)
        elif key == Keypad.KEY_B:  # Set Temperature set_point
            self._set_next_state(SetThermalTarget(self.tank_controller, self), True)
    ...
    def select_view(self):
        ...
        if 0 <= self.level2 < len(self.view_menu_actions):
            ...
            self._set_next_state(
                next_state_class(self.tank_controller, previous_state=self), True
            )
